...
// The result types below mirror the execution-apis schemas in
// src/schemas/block-access-list.yaml: change entries carry a 32-bit
// block-access index plus the post-state value, storage slots pair a 32-byte
// key with the per-index writes, and balance values are 256-bit quantities
// while storage/read values are 32-byte quantities.
